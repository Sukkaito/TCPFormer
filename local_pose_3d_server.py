...
def extract_pose_data_3d(video_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with mp_pose.Pose(
            ...
            while cap.isOpened():
                ...
                if res.pose_landmarks:
                    ...
                    # Bỏ check visibility, luôn lấy tọa độ
                    ...
                    # Forward fill nếu frame bị mất pose nhưng đã có pose trước đó
                    if len(h36m_2d_sequence) > 0:
                        h36m_2d_sequence.append(h36m_2d_sequence[-1])
                        valid_frames_meta.append({
                            "frame_index": int(frame_index),
                            "timestamp": round(frame_index / fps, 3)
                        })
        ...
        if len(h36m_2d_sequence) > 0 and tcpformer_model is not None:
            ...
            output_3d_np = np.array(output_3d_seq)

            # Apply Savitzky-Golay filter to smooth the 3D coordinates over time
            # shape of output_3d_np is (num_frames, 17, 3)
            if num_frames >= 5: # need enough frames for window_length
                window_length = min(31, num_frames) # Có thể tinh chỉnh window_length tùy FPS
                if window_length % 2 == 0:
                    window_length -= 1
                polyorder = min(3, window_length - 1)
                if polyorder > 0:
                    output_3d_np = savgol_filter(output_3d_np, window_length=window_length, polyorder=polyorder, axis=0)

            for i in range(num_frames):
                kp3d = h36m_to_kp3d(output_3d_np[i])
                angles = {
